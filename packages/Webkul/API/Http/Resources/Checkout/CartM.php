...
class CartM extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        return [
            ...
            'items'                     => CartItemM::collection($this->items),
            'selected_shipping_rate'    => new CartShippingRate($this->selected_shipping_rate),
            'payment'                   => new CartPayment($this->payment),
            'billing_address'           => new CartAddressM($this->billing_address),
            'shipping_address'          => new CartAddressM($this->shipping_address),
            'created_at'                => $this->created_at,
            'updated_at'                => $this->updated_at,
            'taxes'                     => ($taxes && isset($taxes[0])) ? $taxes[0] : 0, 
            'formated_taxes'            => ($formatedTaxes && isset($formatedTaxes[0])) ? $formatedTaxes[0] : 0 , 
            'base_taxes'                => ($baseTaxes && isset($baseTaxes[0])) ? $baseTaxes[0] : 0 , 
            'formated_base_taxes'       => ($formatedBaseTaxes && isset($formatedBaseTaxes[0])) ? $formatedBaseTaxes[0] : 0 , 
